...
describe("ScenarioEngine", () => {
  ...
  it("terminates and writes a response for random-error", async () => {
    const store = new StateStore();
  ...
  it("applies scenarios in fixed priority order (delay before random-error)", async () => {
    const store = new StateStore();
    // register random-error first to prove ordering isn't registration order
    ...
    // delay must still be pending its timer — random-error (lower priority) can't
  ...
  it("skips a scenario when its rate roll misses", async () => {
    const store = new StateStore();
    store.register({ type: "random-error", rate: 0.3 });
    const engine = new ScenarioEngine(store);
    const { res, calls } = fakeResponse();
